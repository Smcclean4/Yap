          ...
          return (
            <div className="w-full h-fit max-w-xs bg-gray-800 text-white m-8 flex flex-col rounded-tr-3xl rounded-tl-3xl rounded-bl-3xl" key={idx}>
              ...
              <div className="flex justify-end items-end flex-grow m-4">
                <FontAwesomeIcon className="m-2 cursor-pointer" icon={faHeart} onClick={() => likeYap({ user: String(session?.user.email), id: allYaps.id /* maybe use yapId to identify the specific yap? because yap id refers to yap as a whole .. reference yap tRPC call..  */ })} color={uniqueYaps?.map(val => val.id).includes(allYaps.id) ? "red" : "white"} size="xl" />
              </div>
            </div>
          )
        })}
